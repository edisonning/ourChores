...
- 按用户提供的 `scale-v2-assets.zip` 替换网页及小程序的天平视觉，直接复用四张透明 PNG。支架固定，横梁围绕素材转轴平滑旋转，两侧托盘反向旋转抵消倾斜并跟随吊点移动。
- 积分统计、周期选择、最大 18° 倾角和空状态保持不变。浏览器已验证过渡中托盘水平、左右最大倾角、归零与 320px／390px／1440px 显示；16 项测试通过。
- 小程序正式构建已生成，四张图与用户原素材逐一校验一致；尚未上传微信平台或真机验收。
- 本次网页旧版本备份：`/home/edison/our-chores/backups/scale-v2-20260911/dist`。无需修改后端或数据库。
- 已上线并验证公网 `/stats`、JS、CSS 及四张图片均与本次构建 SHA-256 一致，服务 active。

## 家务天平发布（2026-09-11）

- 网页「统计」入口和页面改为「天平」，保留 `/stats` 路由与原统计 API。按本周／本月已确认积分称重，最多倾斜 18°，显示双方积分、完成件数与差值；兑换不影响称重。
- 小程序同步修改并完成 `miniapp/dist/build/mp-weixin` 构建，尚未上传微信平台或进行真机验收。
- 发布前网页备份：`/home/edison/our-chores/backups/scale-20260911/dist`。先上传新哈希资源，再原子替换 `index.html`，旧资源保留供已打开的页面使用。
- 后端与数据库无需变更、无需重启。16 项测试通过；浏览器验证覆盖 390px、320px 和桌面，以及周期切换、倾斜方向、空记录、平衡、错误重试与减少动态效果。浏览器测试使用本地模拟数据。
- 公网 `/stats`、新 JavaScript 和 CSS 均已通过 HTTPS 获取，并逐文件校验与本地构建的 SHA-256 一致；线上服务状态 active。
- 回滚可将备份的 `index.html` 复制回 `web/dist/index.html`，保留业务数据库。

## 预设更新（2026-09-11）

- 已发布 `db.js`、`reward-presets.js`、`task-presets.js`，新增 12 个心愿与 12 个任务；服务已重新启动，状态 active。
- 更新前通过 SQLite backup API 备份至 `/home/edison/our-chores/backups/presets-20260911-143702/`，同时保存原 `db.js`。
- 验收：任务由 3 项增至 15 项，心愿由 0 项增至 12 项；逐行比对原用户、任务、心愿、打卡、兑换及 meta 记录均保留，数据库完整性检查通过。
- 公网主页 HTTP 200，未登录访问心愿 API 返回 401。登录后的页面未代填 PIN 验收。
- 此次未改前端和依赖。后续部署需同步服务端导入的模块，不能只复制 `db.js`。回滚代码时保留线上业务数据库。
